...
constant int64_t  input_storage_offset   [[function_constant(0)]];
constant int64_t  output_storage_offset  [[function_constant(1)]];
constant uint32_t num_indices            [[function_constant(2)]];
...
kernel void index_select(device const IndexAB & indexAB           [[buffer(0)]],
    ...
    device T * in  = (device T*)((device char*)inputData  + offsets[thread_index].y + offset + input_storage_offset * sizeof(T));
    *out = *in;
}
...
kernel void index_put(device const IndexAB & indexAB           [[buffer(0)]],
    ...
    device T * out = (device T*)((device char*)outputData + offsets[thread_index].x + offset + output_storage_offset * sizeof(T));
    device T * in  = (device T*)((device char*)inputData  + offsets[thread_index].y + input_storage_offset * sizeof(T));
    *out = *in;
}
