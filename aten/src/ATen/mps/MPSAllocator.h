...
public:
  ...
  constexpr static double default_low_watermark_ratio = 0.0;

  explicit MPSHeapAllocatorImpl() :
    m_device(at::mps::MPSDevice::getInstance()->device()),
    m_large_pool_shared(m_device, UsageFlags::SHARED | UsageFlags::HAZARD),
    m_large_pool_private(m_device, UsageFlags::HAZARD),
    m_small_pool_shared(m_device, UsageFlags::SMALL | UsageFlags::SHARED | UsageFlags::HAZARD),
    m_small_pool_private(m_device, UsageFlags::SMALL | UsageFlags::HAZARD),
    // no Hazard Tracking required for the Scalar pool
    m_scalar_pool(m_device, UsageFlags::SMALL | UsageFlags::SCALAR | UsageFlags::SHARED),
  ...
  id<MTLBuffer> getScalarBufferWithValue(void* value, size_t size, MPSStream* stream);
  bool getDebugVerbosity() const { return m_debug_verbosity; }
  size_t getMaxTotalAllowedSize() const { return m_max_total_allowed_size; }
  uint32_t getLowWatermarkInGB() const { return get_low_watermark_in_gb(); }
...
private:
  ...
  // lower ratio to attempt limiting memory allocations up to the lower watermark level by
  // committing command buffers more or less frequently (a.k.a, adaptive commit)
  // value between 0 to m_high_watermark_ratio (0.0 disables adaptive commit, and will commit always)
  // (e.g., value 0.9 means we 'attempt' to limit allocations up to 90% of total memory via adaptive committing)
  double m_low_watermark_ratio;
  // low watermark size limit (in Giga Bytes) at the time we initialize the allocator
  uint32_t m_low_watermark_limit_in_gb;
  ...
  size_t get_allocation_size(size_t Length, uint32_t usage) {
  ...
  uint32_t get_low_watermark_in_gb() const {
    // if low watermark is disabled we
    if (m_low_watermark_ratio == 0.0)
      return 1;
    // current_allocated_size could go beyond max_device_size (e.g., when we swap to disk)
    const uint32_t available_size_in_gb = std::max<ssize_t>(0, (ssize_t)(max_device_size() - current_allocated_size())) / 1073741824L;
    return std::max<uint32_t>(1, static_cast<uint32_t>((double)(available_size_in_gb) * m_low_watermark_ratio));
  }
...
// get a buffer from a specialized pool to import CPU scalars into GPU
