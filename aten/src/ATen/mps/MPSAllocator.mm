...
void MPSHeapAllocatorImpl::init_allocator_params()
{
  ...
  const double high_watermark_limit = m_high_watermark_ratio == 0.0 ? 1.0 : m_high_watermark_ratio;
  ...
  // we keep this initial value and use it to adjust the rate of adaptive committing
  m_low_watermark_limit_in_gb = get_low_watermark_in_gb();
}
    ...
    if (_getAllocImpl().getDebugVerbosity()) {
      if (!(m_usage & HeapAllocator::UsageFlags::SHARED) || m_has_unified_memory) {
        ...
        std::cerr << "Initializing "
                  ...
                  // Adaptive Commit Frequency (ACF) determined based on low watermark ratio
                  << ", ACF: " << _getAllocImpl().getLowWatermarkInGB() << ")\n";
      }
    }
  }
...
uint32_t get_adaptive_commit_frequency() {
  return _getAllocImpl().getLowWatermarkInGB();
}
