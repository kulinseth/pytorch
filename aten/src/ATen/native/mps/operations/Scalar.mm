...
bool scalar_ops_mps(const std::string& op, const Tensor& self, const Tensor& other,
  ...
  const void* selfBuf = self.storage().data();
  const void* otherBuf = other.storage().data();
  const void* outputBuf = output.storage().data();
  const void* selfCpuPtr = selfBuf;
  const void* otherCpuPtr = otherBuf;
  const void* outputCpuPtr = outputBuf;
  ...
  if (scalarOpCategory == ScalarOpCategories::BINARY_OPS) {
    ...
    AT_DISPATCH_ALL_TYPES_AND2(kHalf, kBool, selfDType, "scalar_self_cast", [&]() {
      selfScalar = Scalar(*(static_cast<const scalar_t*>(selfCpuPtr)));
    });
    AT_DISPATCH_ALL_TYPES_AND2(kHalf, kBool, otherDType, "scalar_other_cast", [&]() {
      otherScalar = Scalar(*(static_cast<const scalar_t*>(otherCpuPtr)));
    });
  ...
  // bitwise ops only support integral types
    ...
    AT_DISPATCH_INTEGRAL_TYPES_AND(kBool, commonDType, "scalar_bitwise_ops", [&]() {
      *((scalar_t*)outputCpuPtr) = scalar_bitwise_ops_impl<const scalar_t>(scalarOpType,
                                        static_cast<const scalar_t*>(selfCpuPtr),
                                        static_cast<const scalar_t*>(otherCpuPtr));
    });
  } else {
