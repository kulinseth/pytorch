...
kernel void cross(constant void     * input_        [[buffer(0)]],
    ...
    out[0 * outStride] = input[1 * inputStride] * other[2 * otherStride] - input[2 * inputStride] * other[1 * otherStride];
    out[1 * outStride] = input[2 * inputStride] * other[0 * otherStride] - input[0 * inputStride] * other[2 * otherStride];
    out[2 * outStride] = input[0 * inputStride] * other[1 * otherStride] - input[1 * inputStride] * other[0 * otherStride];
}

#define REGISTER_CROSS(DTYPE)                            \
template                                                 \
[[host_name("cross_" #DTYPE)]]                           \
kernel void cross<DTYPE>(                                \
...
REGISTER_CROSS(float);
REGISTER_CROSS(half);
REGISTER_CROSS(int);
REGISTER_CROSS(long);
REGISTER_CROSS(short);
REGISTER_CROSS(char);
REGISTER_CROSS(uchar);
REGISTER_CROSS(bool);
